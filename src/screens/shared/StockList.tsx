...
import { getAllStock, deleteStock, countAlertes } from '../../database/stockService';
...
const StockList: React.FC<Props> = ({ navigation, route }) => {
  ...
  const appliquerFiltres = (data: StockItem[], terme: string, alertOnly: boolean) => {
    let res = [...data];
    if (alertOnly) res = res.filter(p => p.quantite <= p.seuil_alerte);
    if (terme.trim()) res = res.filter(p => p.nom.toLowerCase().includes(terme.toLowerCase()));
    setFiltered(res);
  };
  ...
  useFocusEffect(useCallback(() => { chargerStock(); }, [chargerStock]));
  ...
  const renderItem = ({ item }: { item: StockItem }) => {
    ...
    const pct      = Math.min(100, Math.round((item.quantite / Math.max(item.seuil_alerte * 3, 1)) * 100));
    ...
    return (
      <View style={[styles.card, enAlerte && styles.cardAlerte]}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate('StockDetail', { produitId: item.id, role })}
        >
          <View style={styles.cardHeaderLeft}>
            <View style={[styles.statusDot, epuise ? styles.dotEpuise : enAlerte ? styles.dotAlerte : styles.dotOk]} />
            <Text style={styles.cardNom} numberOfLines={1}>{item.nom}</Text>
          </View>
          <View style={[styles.badge, epuise ? styles.badgeEpuise : enAlerte ? styles.badgeAlerte : styles.badgeOk]}>
            <Text style={[styles.badgeText, epuise ? styles.badgeTextEpuise : enAlerte ? styles.badgeTextAlerte : styles.badgeTextOk]}>
        ...
        <View style={styles.qRow}>
          <Text style={[styles.qVal, epuise ? styles.qEpuise : enAlerte ? styles.qAlerte : styles.qOk]}>
            {item.quantite}
          </Text>
        ...
        <View style={styles.jauge}>
          <View style={[
            styles.jaugeFill,
            { width: `${pct}%` as any, backgroundColor: epuise ? '#F85149' : enAlerte ? '#f59e0b' : G },
          ]} />
        </View>
        ...
        <View style={styles.actions}>
          {isSecretaire ? (
            ...
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('StockRetirer', { produit: item })}>
              <Text style={styles.actionRetirerText}>➖ Marquer utilisation</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            ...
            <Text style={styles.headerSub}>
              {isSecretaire ? 'Secrétaire — Gestion complète' : 'Médecin — Consultation'}
            </Text>
          </View>
          {alerteCount > 0 && (
            <View style={styles.alerteBubble}>
              <Text style={styles.alerteBubbleNum}>{alerteCount}</Text>
              <Text style={styles.alerteBubbleLabel}>alerte{alerteCount > 1 ? 's' : ''}</Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={[styles.statPill, alerteCount > 0 && styles.statPillAlerte]}>
            <Text style={[styles.statPillNum, alerteCount > 0 && { color: '#f59e0b' }]}>{alerteCount}</Text>
            <Text style={[styles.statPillLabel, alerteCount > 0 && { color: '#f59e0b' }]}>alertes</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.searchRow}>
        ...
        <TouchableOpacity
          ...
          <Text style={[styles.filterBtnText, filterAlerte && styles.filterBtnTextActive]}>
            ⚠️ Alertes
          </Text>
        </TouchableOpacity>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyDesc}>{search ? 'Recherche infructueuse.' : 'Stock vide.'}</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  alerteBubble: { backgroundColor: '#2d1f00', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 8, alignItems: 'center', borderWidth: 1, borderColor: '#f59e0b' },
  alerteBubbleNum:   { fontSize: 20, fontWeight: '800', color: '#f59e0b' },
  alerteBubbleLabel: { fontSize: 10, color: '#f59e0b', fontWeight: '600' },
  statsRow:     { flexDirection: 'row', gap: 8 },
  statPill:     { flex: 1, backgroundColor: '#0D1117', borderRadius: 10, padding: 10, alignItems: 'center', borderWidth: 1, borderColor: '#30363D' },
  ...
  searchBox:    { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: '#161B22', borderRadius: 12, borderWidth: 1.5, borderColor: '#30363D', paddingHorizontal: 12 },
  searchIcon:   { fontSize: 15, marginRight: 6 },
  searchInput:  { flex: 1, paddingVertical: 11, fontSize: 14, color: '#E6EDF3' },
  filterBtn:    { backgroundColor: '#161B22', borderRadius: 12, borderWidth: 1.5, borderColor: '#30363D', paddingHorizontal: 14, justifyContent: 'center' },
  ...
  card:         { backgroundColor: '#161B22', borderRadius: 16, marginBottom: 12, padding: 16, borderWidth: 1, borderColor: '#30363D' },
  ...
  jauge:        { height: 6, backgroundColor: '#30363D', borderRadius: 3, overflow: 'hidden', marginBottom: 12 },
  jaugeFill:    { height: '100%', borderRadius: 3 },
  actions:      { flexDirection: 'row', gap: 8 },
  actionBtn:    { borderRadius: 10, paddingVertical: 10, paddingHorizontal: 12, alignItems: 'center', justifyContent: 'center' },
  actionArrivage:     { backgroundColor: '#0d2818', borderWidth: 1, borderColor: G },
  actionArrivageText: { fontSize: 13, fontWeight: '700', color: G },
  actionEdit:         { backgroundColor: '#1C2128', borderWidth: 1, borderColor: '#388BFD' },
  actionEditText:     { fontSize: 16 },
  actionDelete:       { backgroundColor: '#1C2128', borderWidth: 1, borderColor: '#F85149' },
  actionDeleteText:   { fontSize: 16 },
  actionRetirer:      { flex: 1.5, backgroundColor: '#2d1f00', borderWidth: 1, borderColor: '#f59e0b' },
  ...
  fab:          { position: 'absolute', bottom: 24, right: 20, width: 60, height: 60, borderRadius: 30, backgroundColor: G, alignItems: 'center', justifyContent: 'center', elevation: 5 },
