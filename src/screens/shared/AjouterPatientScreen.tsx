...
type AjouterPatientRouteProp = RouteProp<
...
const AjouterPatientScreen = () => {
  ...
  // États pour le RDV (Uniquement pour la création d'un nouveau patient)
  ...
  const handleEnregistrer = async () => {
    ...
    // Si une date de RDV est mise, on exige aussi une heure valide
    if (dateRDV.trim() !== '' && !heureRDV.trim()) {
      Alert.alert(
        'Champs requis',
        'Veuillez renseigner une heure pour le rendez-vous (Ex: 14:30).',
      );
      return;
    }
    ...
    if (dateRDV.trim() !== '' && !heureRegex.test(heureRDV)) {
      Alert.alert(
        'Format invalide',
        "L'heure doit être au format HH:MM (Ex: 10:15)",
      );
      return;
    }
    ...
    try {
      if (isEdit) {
        ...
        // 1. Si un RDV est demandé, on vérifie d'abord si le créneau est libre
        let dateHeureComplete = '';
        if (dateRDV.trim() !== '') {
          dateHeureComplete = `${dateRDV}T${heureRDV}:00`;

          // Requête anti-doublon
          ...
          if (rdvExistants.length > 0) {
            ...
            Alert.alert(
              ...
              `L'horaire ${heureRDV} est déjà réservé pour le patient : ${autrePatient.nom} ${autrePatient.prenom}. Veuillez choisir une autre heure.`,
            );
            return;
          }
        }

        // 2. Insérer le patient
        ...
        `,
          [
            ...
            sexe,
            dateN,
            role === 'medecin' ? antecedents : '',
          ],
        );

        // 3. Créer le RDV lié avec l'heure personnalisée
        if (dateRDV.trim() !== '') {
          await ajouterRDV({
        ...
        Alert.alert(
          'Succès',
          'Patient enregistré' +
            (dateRDV ? ` avec son RDV à ${heureRDV}.` : '.'),
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.header}>
          <Text style={styles.title}>
            {isEdit ? '✏️ Modifier Patient' : '👤 Nouveau Patient'}
          </Text>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.genderContainer}>
            ...
            >
              <Text
                style={[
                  styles.genderText,
                  sexe === 'M' && styles.genderTextActive,
                ]}
              >
                Homme 👨
              </Text>
            ...
            >
              <Text
                style={[
                  styles.genderText,
                  sexe === 'F' && styles.genderTextActive,
                ]}
              >
                Femme 👩
              </Text>
          ...
          {!isEdit && (
            <View style={styles.rdvBox}>
              <Text style={[styles.label, { color: '#00BFA5' }]}>
                📅 Fixer un RDV (AAAA-MM-JJ) - Optionnel
              </Text>
              <TextInput
                style={[
                  styles.input,
                  { borderColor: '#00BFA5', marginBottom: 12 },
                ]}
                value={dateRDV}
...
// 🔥 👇 AJOUT DE LA FONCTION DE FORMATAGE AUTOMATIQUE EXTRÈMEMENT ROBUSTE 👇
const formatHeure = (texte: string): string => {
  // On supprime tout ce qui n'est pas un chiffre
  const chiffres = texte.replace(/\D/g, '');

  // Formatage en temps réel selon le nombre de chiffres saisis
