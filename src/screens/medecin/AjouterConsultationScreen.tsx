...
import {
  ...
  ScrollView, Alert, FlatList, StatusBar
} from 'react-native';
import { useRoute, useNavigation } from '@react-navigation/native';
import { enregistrerConsultation, MedicamentPrescrit } from '../../database/rdvService';
...
const AjouterConsultationScreen = () => {
  ...
  const ajouterMedicamentALaListe = () => {
    ...
    const nouveau = { nom_medicament: nomMed, posologie, remarques_medicament: conseil };
    setMedicaments([...medicaments, nouveau]);
    // Reset les champs
    setNomMed(''); setPosologie(''); setConseil('');
  };
            ...
            <div style="margin-top: 20px;">
              ${meds.map(m => `
                <div style="margin-bottom: 15px;">
                  ...
                  ${m.remarques_medicament ? `<p style="margin-left: 20px; font-style: italic;">Note : ${m.remarques_medicament}</p>` : ''}
                </div>
              `).join('')}
            </div>
  ...
  const handleFinaliser = async () => {
    ...
    try {
      ...
      await enregistrerConsultation(patientId, symptomes, remarquesMedecin, medicaments);
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.section}>
          ...
          />
          <TouchableOpacity style={styles.btnAddMed} onPress={ajouterMedicamentALaListe}>
        ...
        {medicaments.map((item, index) => (
          <View key={index} style={styles.medItem}>
            <Text style={{color: '#E6EDF3'}}>{item.nom_medicament} - {item.posologie}</Text>
            <TouchableOpacity onPress={() => setMedicaments(medicaments.filter((_, i) => i !== index))}>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 22, fontWeight: 'bold', color: '#E6EDF3', marginBottom: 5 },
  patientName: { fontSize: 16, color: '#00BFA5', marginBottom: 20 },
  section: { backgroundColor: '#161B22', padding: 15, borderRadius: 10, marginBottom: 20 },
  titleSection: { color: '#E6EDF3', fontSize: 18, marginBottom: 10, fontWeight: 'bold' },
  label: { color: '#8B949E', marginBottom: 5, fontSize: 14 },
  input: { backgroundColor: '#0D1117', color: '#E6EDF3', padding: 10, borderRadius: 5, marginBottom: 15, textAlignVertical: 'top', height: 80, borderWidth: 1, borderColor: '#30363D' },
  smallInput: { backgroundColor: '#0D1117', color: '#E6EDF3', padding: 10, borderRadius: 5, marginBottom: 10, borderWidth: 1, borderColor: '#30363D' },
  btnAddMed: { backgroundColor: '#30363D', padding: 12, borderRadius: 5, alignItems: 'center' },
  btnAddMedText: { color: '#58A6FF', fontWeight: 'bold' },
  medItem: { flexDirection: 'row', justifyContent: 'space-between', padding: 10, borderBottomWidth: 1, borderBottomColor: '#30363D' },
  btnFinaliser: { backgroundColor: '#238636', padding: 18, borderRadius: 10, alignItems: 'center', marginTop: 20, marginBottom: 40 },
