...
import { getDatabase, executeInsert } from '../../database/database';
...
export default function LoginScreen({ navigation, onLogin }: any) {
  ...
  const handleLogin = async () => {
    ...
    let hasError = false;
    if (!email.trim()) {
      setErreurEmail("L'email est obligatoire.");
      hasError = true;
    ...
    if (!mdp.trim()) {
      setErreurMdp('Le mot de passe est obligatoire.');
      hasError = true;
    }
    if (hasError) return;
    ...
    try {
      ...
      if (!authResult.emailVerifie) {
        setErreurGeneral(
          "Votre compte n'est pas encore valide. Verifiez vos e-mails.",
        );
        setLoading(false);
        return;
      }
      ...
      if (!user) {
        await executeInsert(
          `INSERT OR IGNORE INTO users (role, nom, prenom, email, password, sexe) VALUES (?, ?, ?, ?, ?, ?);`,
          [
            'medecin',
            emailNormalise.split('@')[0],
            '',
            emailNormalise,
            mdpNormalise,
            'M',
      ...
      if (!user) throw new Error('DB_FAILED');

      const userData = { ...user, emailVerifie: true };
      await AsyncStorage.setItem('user', JSON.stringify(userData));
      setSucces('Connexion reussie !');
      ...
      if (
        ...
        setErreurGeneral('Trop de tentatives. Veuillez patienter.');
      } else if (code === 'auth/network-request-failed') {
        setErreurGeneral('Probleme de connexion. Verifiez votre Internet.');
      } else {
        setErreurGeneral('Une erreur est survenue. Veuillez reessayer.');
      }
    } finally {
      setLoading(false);
    }
        ...
        <View style={styles.form}>
          ...
          <View style={styles.fieldWrap}>
            ...
            <TextInput
              style={[
                styles.input,
                erreurEmail ? styles.inputError : undefined,
              ]}
            ...
            />
            {erreurEmail !== '' && (
              <Text style={styles.erreurTxt}>{erreurEmail}</Text>
            )}
          </View>
          ...
          <View style={styles.fieldWrap}>
            ...
            {erreurMdp !== '' && (
              <Text style={styles.erreurTxt}>{erreurMdp}</Text>
            )}
          </View>
          ...
          >
            <Text style={styles.btnOublieTxt}>Mot de passe oublie ?</Text>
          </TouchableOpacity>
          ...
          >
            <Text style={styles.btnCreerTxt}>
              ...
              <Text style={{ color: '#00BFA5' }}>Creer un compte</Text>
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
