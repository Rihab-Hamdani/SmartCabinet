...
const MedecinTabs = () => {
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        headerShown: false,
        tabBarStyle: { backgroundColor: '#161B22', borderTopColor: '#30363D', height: 70, paddingBottom: 10, paddingTop: 8 },
        tabBarActiveTintColor: '#00BFA5',
        tabBarInactiveTintColor: '#8B949E',
        tabBarIcon: ({ color }) => {
          const icons: Record<string, string> = { Dashboard: '🏠', Patients: '👥', RDV: '📅', Stock: '📦' };
...
const SecretaireTabs = () => {
  return (
    ...
    >
      <Tab.Screen name="Dashboard" component={DashboardSecretaire} />
...
export default function AppNavigator() {
  ...
  // Cette fonction s'exécute à chaque fois que la navigation bouge d'un écran à un autre
  const verifierSessionDeconnexion = React.useCallback(async () => {
  ...
  return (
    <NavigationContainer onStateChange={verifierSessionDeconnexion}>
      {user && user.role ? (
        <Stack.Navigator screenOptions={{ headerShown: false }}>
          {user.role === 'medecin' ? (
            <Stack.Screen name="MedecinMain" component={MedecinTabs} />
          ) : (
            <Stack.Screen name="SecretaireMain" component={SecretaireTabs} />
          )}
        </Stack.Navigator>
      ) : (
        <Stack.Navigator screenOptions={{ headerShown: false }}>
